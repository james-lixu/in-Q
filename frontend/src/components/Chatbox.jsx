...
const Chatbox = ({ friend, conversationId, onClose }) => {
  ...
  return (
    <div className="chat-box relative w-72 bg-gray-800 text-white p-4 rounded-lg shadow-lg m-4">
      <div className="chat-header flex justify-between items-center border-b pb-2 mb-2">
        <h3 className="font-bold">{friend.name}</h3>
        <button onClick={onClose} className="text-red-500">Close</button>
      </div>

      <div className="chat-messages h-48 overflow-y-auto bg-gray-900 p-2 rounded" ref={chatMessagesRef}>
        {messages.map((msg, index) => {
          ...
          return (
            <div key={index} className={`flex ${isSender ? "justify-end" : "justify-start"} mb-2`}>
              <p className={`text-sm p-2 rounded-lg ${isSender ? "bg-blue-500 text-white" : "bg-gray-600 text-white"} max-w-xs`}>
